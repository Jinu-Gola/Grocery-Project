...
function Login() {
  ...
  const data = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8080/regis', {
        name: name,
        email: emails,
        mobile: mobile,
        password: pass,
      ...
      if (response.data.status === 1) {
        ...
        setError(true);
        alert("Signup unsuccessful: " + response.data.message);
      }
    } catch (error) {
      if (error.response && error.response.status === 401) {
        alert("Signup unsuccessful: " + error.response.data.message);
      } else {
                ...
                step === 0 ? (<form className="sign-in-htm" onSubmit={handleFormSubmit}>
                  ...
                  <div className="container">
                    <div className="wrapper d-flex mx-5">
                      ...
                      <div >
                        <button
                          className="btn btn-white mx-5 mt-3 bg-white"
                          // className="icon"
                          // className="button "
                          onClick={initializeCaptcha}
                  ...
                  {err && <div className="error1">{err}</div>} {/* Error message */}
